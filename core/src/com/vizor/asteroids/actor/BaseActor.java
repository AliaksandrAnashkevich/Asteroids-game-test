...
/**
 * Extends functionality of the LibGDX Actor class. by adding support for textures/animation, collision polygons,
 * movement, world boundaries, and camera scrolling. Most game objects should extend this class; lists of extensions can
 * be retrieved by stage and class name.
 *
...
public abstract class BaseActor extends Group {
    ...
    /**
     * Repositions this BaseActor so its center is aligned with center of other BaseActor. Useful when one BaseActor
     * spawns another.
     *
    ...
    /**
     * Checks if animation is complete: if play mode is normal (not looping) and elapsed time is greater than time
     * corresponding to last frame.
     *
     * @return
     */
    ...
    /**
     * Set the speed of movement (in pixels/second) in current direction. If current speed is zero (direction is
     * undefined), direction will be set to 0 degrees.
     *
    ...
    /**
     * Update accelerate vector by angle and value stored in acceleration field. Acceleration is applied by
     * <code>applyPhysics</code> method.
     *
    ...
    /**
     * Update accelerate vector by current rotation angle and value stored in acceleration field. Acceleration is
    ...
    /**
     * Adjust velocity vector based on acceleration vector, then adjust position based on velocity vector. <br> If not
     * accelerating, deceleration value is applied. <br> Speed is limited by maxSpeed value. <br> Acceleration vector
    ...
    /**
     * Set rectangular-shaped collision polygon. This method is automatically called when animation is set, provided
     * that the current boundary polygon is null.
     *
     * @see #setAnimation
     */
    ...
    /**
     * Replace default (rectangle) collision polygon with an n-sided polygon. <br> Vertices of polygon lie on the
     * ellipse contained within bounding rectangle. Note: one vertex will be located at point (0,width); a 4-sided
    ...
    /**
     * Implement a "solid"-like behavior: when there is overlap, move this BaseActor away from other BaseActor along
     * minimum translation vector until there is no overlap.
     *
     * @param other BaseActor to check for overlap
    ...
    /**
     * If an edge of an object moves past the world bounds, adjust its position to keep it completely on screen.
     */
    ...
    /**
     * Center camera on this object, while keeping camera's range of view (determined by screen size) completely within
     * world bounds.
     */
    public void alignCamera() {
        ...
        cam.position.x = MathUtils
            .clamp(cam.position.x, cam.viewportWidth / 2, worldBounds.width - cam.viewportWidth / 2);
        cam.position.y = MathUtils
            .clamp(cam.position.y, cam.viewportHeight / 2, worldBounds.height - cam.viewportHeight / 2);
        cam.update();
    }
    ...
    /**
     * Retrieves a list of all instances of the object from the given stage with the given class name or whose class
     * extends the class with the given name. If no instances exist, returns an empty list. Useful when coding
    ...
    /**
     * Draws current frame of animation; automatically called by draw method in Stage class. <br> If color has been set,
     * image will be tinted by that color. <br> If no animation has been set or object is invisible, nothing will be
     * drawn.
     *
